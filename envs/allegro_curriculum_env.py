    ...
    def step(self, action):
        ...
        if self.stage == 1:
            ...
            if hasattr(self, "prev_finger_dist") and self.prev_finger_dist is not None:
                reward += 8.0 * (self.prev_finger_dist - finger_dist)
        ...
        if self.stage == 2:
            ...
            reward += 0.5 * self.pinch_hold_steps
        ...
        if self.stage == 4:
            ...
            # Preserve pinch skill.
            reward += -1.5 * thumb_dist
            reward += -1.0 * index_dist
            ...
            # Lift reward starts after true pinch.
            lift_amount = max(0.0, float(block_pos[2]) - 0.058)

            if true_pinch:
                reward += 40.0 * lift_amount

            # Penalize falling below table height.
            if block_pos[2] < 0.04:
                reward -= 2.0

            # Success = pinch + small lift.
            success = bool(
                true_pinch
                and self.pinch_hold_steps >= 3
                and block_pos[2] > 0.068
            )
            ...
            info = {
                ...
                "thumb_contact": bool(thumb_contact),
                "index_contact": bool(index_contact),
                "lifting": bool(block_pos[2] > 0.068),
                "stable_hold": bool(False),
                "success": bool(success),
                "block_pos": block_pos.copy(),
                "block_quat": block_quat.copy(),
