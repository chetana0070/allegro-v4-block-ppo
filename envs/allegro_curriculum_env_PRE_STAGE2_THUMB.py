...
class AllegroBlockEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # -------------------------------------------------
        # Curriculum Stage 2: thumb + finger pinch
        # -------------------------------------------------
        if self.stage == 2:
            reward = 0.0

            # Keep reaching behavior from Stage 1.
            reward += -3.0 * finger_dist

            if hasattr(self, "prev_finger_dist") and self.prev_finger_dist is not None:
                reward += 10.0 * (self.prev_finger_dist - finger_dist)
            ...
            if index_contact or middle_contact or ring_contact:
                reward += 2.0

            if pinch_grasp:
                reward += 12.0
                if not hasattr(self, "pinch_hold_steps"):
                    self.pinch_hold_steps = 0
                self.pinch_hold_steps += 1
            else:
                self.pinch_hold_steps = 0

            # Sustained pinch matters more than one-frame contact.
            reward += 1.0 * self.pinch_hold_steps

            # Do not reward lift yet. Keep cube stable.
